...
- **Rights & Pricing:** all territories (the Amazon.in import listing is a harmless ghost SKU; Notion Press wins there on price and delivery). Floor = print cost ÷ 0.6; the Pricing tab shows the minimum per marketplace once the interior is processed — read it there rather than computing it. Reach pricing: at or just above the floor, reported per marketplace. **Expanded Distribution: off** (it sends the book to Ingram under KDP's terms; with an own ISBN it collides with a later IngramSpark listing of that ISBN).
- **Before Publish:** Launch Previewer and fix anything it flags; order a proof if the author wants one. Publish is the human's click.
